...
public class CloudSimExample1 {
	...
	public static void main(String[] args) {
		...
		try {
			// S1: Initialize the CloudSim package. It should be called before creating any entities.
			 ...
			 * initialize() creates two queues - a Queue of SimEvents (future) and another Queue of SimEvents (deferred延迟).
			 ...
			 * (with num_user as 1, its name as CloudSimShutDown, id as -1, and state as RUNNABLE). Then this new entity is added to the simulation
			...
			// S5: Create one Cloudlet
			cloudletList = new ArrayList<Cloudlet>();

			// Cloudlet properties
			int id = 0;
			long length = 400000; //执行时应用长度
			long fileSize = 300;
			long outputSize = 300;
			UtilizationModel utilizationModel = new UtilizationModelFull(); //利用率
	...
	private static Datacenter createDatacenter(String name) {
		...
		// 2. A Machine contains one or more PEs or CPUs/Cores.  CPU列表
		...
		String arch = "x86"; // system architecture 服务器结构
		String os = "Linux"; // operating system  操作系统
		String vmm = "Xen";		// 虚拟机
		double time_zone = 10.0; // time zone this resource located 资源所在时区
		double cost = 3.0; // the cost of using processing in this resource 使用资源花费的进程
		double costPerMem = 0.05; // the cost of using memory in this resource 使用资源花费的内存
		double costPerStorage = 0.001; // the cost of using storage in this 使用资源花费的存储
										// resource
		double costPerBw = 0.0; // the cost of using bw in this resource 使用资源花费的带宽
		...
		// 6. Finally, we need to create a PowerDatacenter object. 数据中心能耗
	...
	private static void printCloudletList(List<Cloudlet> list) {
		...
		DecimalFormat dft = new DecimalFormat("###.##");//小数格式
